...
export const ParticipantDetailsPage: React.FC<ParticipantDetailsPageProps> = ({
  id,
  participantResource,
}) => {
  const [participantFromServer, setParticipantFromServer] = useState<
  ...
  const [selectedPresence, setSelectedPresence] = useState<number | undefined>(
    undefined,
  )
  ...
  if (participantDetails) {
    ...
    const participantStatus = internationalGuest
      ? 'KV-osallistuja'
      : nonScout
      ? 'EVP'
    ...
    const familyCampSelections = _.groupBy(
      selections,
      (row) => row.kuksaGroupId,
    )

    const renderedFamilyCampSelections = _.map(
      familyCampSelections,
      (selection) => {
        const rows = _.map(selection, (row) => (
          <dd key={row.selectionName}>{row.selectionName}</dd>
        ))
        return (
          <dl key={selection[0].kuksaGroupId}>
            <dt>{_.head(selection)!.groupName}</dt>
            {rows}
          </dl>
        )
      },
      ...
      <>
        ...
        <main className="participant-details-container">
          ...
          <Panel header="Läsnäolo">
            ...
            <form onSubmit={savePresence}>
              <PresenceSelector
                ...
                property="presence"
                label="Muuta tilaa"
                currentSelection={{ presence: selectedPresence }}
              />
              <LoadingButton
          ...
          <Panel header="Allergiat ja erityisruokavaliot">
            {_.isEmpty(allergyNames) && _.isEmpty(diet) ? (
              <p>Ei allergioita</p>
            ) : (
              ''
            )}
          ...
          <Panel header="Leiritoimiston merkinnät">
            ...
            />
            <LoadingButton
              loading={campOfficeNotesSaving}
              onClick={saveCampOfficeNotes}
              label="Tallenna"
              labelWhileLoading="Tallennetaan…"
            />
          </Panel>
          <Panel header="Lisätiedot">
            ...
            />
            <LoadingButton
              loading={editableInfoSaving}
              onClick={saveEditableInfo}
              label="Tallenna"
              labelWhileLoading="Tallennetaan…"
            />
          </Panel>
