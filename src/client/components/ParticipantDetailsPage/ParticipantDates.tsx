...
const Week: React.FC<{
  week: readonly { date: moment.Moment; isParticipating: boolean }[]
}> = ({ week }) => (
  <tr>
    {week.map((day) => (
      <Day key={day.date.toString()} day={day} />
    ))}
  </tr>
)

const Day: React.FC<{
  day: { date: moment.Moment; isParticipating: boolean }
}> = ({ day }) => {
...
export const ParticipantDates: React.FC<ParticipantDatesProps> = (props) => {
  ...
  if (_.isEmpty(participantDays)) {
    return <p>Osallistujalle ei löydy ilmoittautumispäiviä.</p>
  }
  ...
  const weeks: Record<
    string,
    { date: moment.Moment; isParticipating: boolean }[]
  > = {}

  for (
    let d = startOfFirstWeek;
    d <= endOfLastWeek;
    ...
    weeks[currentWeekNum].push({
      date: moment(d),
      isParticipating:
        _.find(participantDays, { date: d.startOf('day').toISOString() }) !==
        undefined,
    })
  }
  ...
  return (
    <table>
      <tbody>
        {_.map(weeks, (row, weekNumber) => (
          <Week key={weekNumber} week={row} />
        ))}
      </tbody>
    </table>
  )
