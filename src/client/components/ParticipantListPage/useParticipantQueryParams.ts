...
export function useParticipantQueryParams(): ParticipantQueryParams {
  ...
  const updateFilter = useCallback((parameterName: string, newValue: unknown) => {
  ...
  const updateOrder = useCallback((newOrder: Ordering) => updateQueryParams({ order: newOrder }), [updateQueryParams]);

  const updateOffset = useCallback((newOffset: number) => updateQueryParams({ offset: newOffset }), [updateQueryParams]);
