...
export const GenericPropertyFilterContainer: React.FC<GenericPropertyFilterContainerProps> =
  ({ label, onChange, properties, currentSelection, optionsByProperty }) => {
      ...
      <div className="generic-property-filter__row">
        <Label label={label}>
          <select value={property} onChange={handlePropertyChanged}>
            <option value=""></option>
            {properties.map(({ property, label }, index) => (
