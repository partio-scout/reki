...
import { ParticipantOverview, OptionsByProperty, AvailableDates } from '../../model';
import { RestfulResource } from '../../RestfulResource';

export type ParticipantListColumn =
  | { type: 'presence' | 'profileLink' | 'date' | 'text'; property: string; label: string }
  | { type: 'iconWithTooltip'; property: string; icon: IconType; label: { icon: IconType; tooltip: string } }
  | { type: 'boolean'; property: string; label: string; true?: string; false?: string }
...
export type QuickFilterDefinition = Readonly<
  ...
  | { type: 'generic'; label: string; properties: readonly Readonly<{ property: string; label: string }>[] }
  | { type: 'availableDates'; label: string }
>
...
export const ParticipantListPage: React.FC<ParticipantListPageProps> = ({ optionResource, participantDateResource, participantResource }) => {
  ...
  const [participants, setParticipants] = useState<readonly ParticipantOverview[]>([]);
  ...
  useEffect((): void => {
    ...
    participantResource.findAll(filterString)
      .then(participantList => Rt.Record({ result: Rt.Array(ParticipantOverview).asReadonly() }).asReadonly().check(participantList))
  ...
  const [checked, setChecked] = useState<readonly ParticipantOverview['participantId'][]>([]);
  ...
  const handleCheckboxChange = (isChecked: boolean, participantId: ParticipantOverview['participantId']) => {
    if (isChecked) {
  ...
  const isChecked = (participantId: ParticipantOverview['participantId']): boolean => checked.includes(participantId);
  ...
  const handleMassEdit = (newValue: number) => {
    participantResource.raw('POST', 'massAssign', { body: { ids: checked, newValue: newValue, fieldName: 'presence' } })
      .then(response => forceReloadParticipants(),
            error => showError('Osallistujien tilan päivitys epäonnistui', { error }));
  };
  ...
  return (
    <>
      ...
      <main className="content-box">
              ...
              <tr>
                ...
                <th><SelectAll hideLabel checked={ checked } participants={ participantIds } setChecked={ setChecked } /></th>
                {
                  participantListColumns.map(column => column.type === 'availableDates' ? (
                    <th key={ column.type } colSpan={ availableDates.length }>{ column.label }</th>
                  ) : (
                    <SortableHeaderCell
        ...
        <MassEdit checked={ checked } participants={ participantIds } setChecked={ setChecked } onSubmit={ handleMassEdit } participantsLoading={ participantsLoading } />
