...
export const ListOffsetSelector: React.FC<ListOffsetSelectorProps> = ({ offset = 0, limit = 1, onOffsetChanged, participantCount = 0 }) => {
  const pageCount = Math.ceil(participantCount/limit);
  const activePageNumber = Math.floor(offset/limit);
  const handleSelect = (event: React.ChangeEvent<HTMLSelectElement>) => onOffsetChanged(Number(event.target.value)*limit);
