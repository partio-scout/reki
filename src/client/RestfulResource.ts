...
export type RestfulResource = {
  ...
  raw: (method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, options?: { body?: unknown; filters?: string }) => Promise<unknown>;
}
...
export function RestfulResource(endpoint: string): RestfulResource {
  ...
  function raw(method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, { body, filters }: { body?: unknown; filters?: string } = {}): Promise<unknown> {
