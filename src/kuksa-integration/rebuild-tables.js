...
async function buildAllergyTable() {
  ...
  const kuksaSelections = await models.KuksaExtraSelection.findAll({ where: { kuksaExtraselectiongroupId: { [Op.in]: _.map(selGroups, group => group.id) } } });
  const selections = kuksaSelections.map(selection => ({ name: selection.name, allergyId: selection.id }));
...
async function rebuildParticipantsTable() {
  ...
  const participants = wrappedParticipants.map(config.getParticipantBuilderFunction());
...
async function addAllergiesToParticipants() {
  async function removeOldAndAddNewAllergies(participant, newAllergies) {
    await models.ParticipantAllergy.destroy( { where: { participantParticipantId: participant.participantId } } );
  ...
  const participants = await models.Participant.findAll({ include: [models.Allergy] });
  for (const participant of participants) {
...
async function addDatesToParticipants() {
  ...
  for (const wrappedParticipant of wrappedParticipants) {
    await models.ParticipantDate.destroy( { where: { participantId: wrappedParticipant.get('id') } } );
    await models.ParticipantDate.bulkCreate(mapPaymentsToDates(wrappedParticipant));
  }
...
async function buildSelectionTable() {
  ...
  for (const p of participants) {
    ...
    const selections = participantSelections
      .filter(s => !!(s.kuksa_extraselection && s.kuksa_extraselection.kuksa_extraselectiongroup)) // Apparently some selections don't have a group, so handle only selections with group
      .filter(s => (_.indexOf(groupsToCreate, s.kuksa_extraselection.kuksa_extraselectiongroup.name) > -1))
      .map(sel => ({
...
async function deleteCancelledParticipants() {
  ...
  const participants = await models.KuksaParticipant.findAll({ where: { cancelled: true } });
  const idsToDelete = _.map(participants, 'id');
  const count = await models.Participant.destroy({ where: { participantId: { [Op.in]: idsToDelete } } });
...
async function buildOptionTable() {
  ...
  for (const field of config.getOptionFieldNames()) {
    const values = await models.Participant.aggregate( field, 'DISTINCT', { plain: false } );

    for (const value of _(values).map(obj => obj['DISTINCT']).uniq().reject(_.isNull).value().sort()) {
