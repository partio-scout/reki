...
describe('Kuksa integration (yes, this is very slow)', () => {
  ...
  it('correctly transfers allergies', () =>
    expect(models.Allergy.findByPk(415)).to.eventually.have.property(
      'name',
      'Herne, kypsä',
    ))

  it('correctly transfers diets', () =>
    expect(models.Allergy.findByPk(406)).to.eventually.have.property(
      'name',
      'Gluteeniton',
    ))
  ...
  it('correctly transfers participants allergies', () =>
    expect(
      models.Participant.findByPk(448, { include: models.Allergy }),
  ...
  it('sets the correct date for participants', () =>
    models.Participant.findByPk(448, { include: { all: true } }).then(
      (participant) => {
        expect(participant).to.have.nested.property('dates[1].date')
        expect(participant.dates[0].date).to.equalDate(
          moment('2016-07-23').toDate(),
        )
        expect(participant.dates[1].date).to.equalDate(
          moment('2016-07-24').toDate(),
        )
      },
    ))
